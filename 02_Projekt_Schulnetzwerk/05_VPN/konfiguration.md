...
Dieses Dokument beschreibt die VPN-Konfiguration anhand der vorhandenen Nachweis-Screenshots. Es handelt sich um eine dokumentierte Zusammenfassung der erkennbaren Konfigurationsbestandteile und nicht um einen vollständigen Copy-Paste-Konfigurationsauszug.
...
Die Extended Access List 100 legt fest, welcher Traffic verschlüsselt werden soll.
...
Der Screenshot von `R-HQ-SCHULE-02` zeigt ausserdem Treffer auf der ACL-Regel. Das bestätigt, dass Verkehr erzeugt wurde, der zur VPN-Regel passt.
...
| --- | --- | --- | --- | --- | --- |
| `R-HQ-SCHULE-01` | `VPN-MAP 10 ipsec-isakmp` | `200.169.1.1` | `100` | im Screenshot sichtbar, nicht vollständig ausgeschrieben | `GigabitEthernet0/0` |
| `R-HQ-SCHULE-02` | `VPN-MAP 10 ipsec-isakmp` | `200.169.2.1` | `100` | im Screenshot sichtbar, nicht vollständig ausgeschrieben | `GigabitEthernet0/0` |
...
Die Ausgabe `show crypto ipsec sa` zeigt die IPsec Security Associations. Auf beiden Routern sind ESP-SAs vorhanden und aktiv. Die Paketzähler zeigen, dass Pakete verschlüsselt und entschlüsselt wurden.

| Router | Lokales Netz | Entferntes Netz | Beobachtung |
| --- | --- | --- | --- |
| `R-HQ-SCHULE-01` | `192.168.0.0/24` | `10.10.0.0/24` | Paketzähler für encrypt/decrypt sichtbar |
| `R-HQ-SCHULE-02` | `10.10.0.0/24` | Richtung Hauptstandort | ESP-SAs und Paketzähler sichtbar |

## Zusammenfassung

Die Konfiguration besteht aus diesen Bausteinen:

1. Routing über das WAN-/Internet-Segment
2. ACL 100 für den interessanten VPN-Traffic
3. ISAKMP/IKE für Phase 1
4. IPsec Transform-Set und Crypto Map für Phase 2
